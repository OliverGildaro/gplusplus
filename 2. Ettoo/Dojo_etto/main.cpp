...
void serialize_children(string &str, const vector<node> &children, int ident, int current_ident)//4,4
{
    for (auto &x : children)
    {
        visit(
...
void serialize_element(string &str, const string &name, 
    const vector<node> &children, const map<string, string> &atributes, 
    int ident, int current_ident)//4,0 //4,4
{
    ...
    if (children.size())
    {
        ...
        serialize_children(str, children, ident, ident != -1 ? ident + current_ident : 0);//4,4
...
void element::serialize(string &str, int ident, int current_ident) const//4,4
{
